...
class DecisionEngine:
    ...
    def __init__(self, policy_mode: str | None = None) -> None:
        ...
        self._policy = _OnnxPolicy(model_path) if self.policy_mode == "policy" else None
...
class _OnnxPolicy:
    ...
    def predict_action(self, observation: dict[str, Any] | None) -> AriaAction | None:
        ...
        vec = _encode_observation(observation)
        if self._input_size:
...
def _encode_observation(observation: dict[str, Any]) -> list[float]:
    inbox = observation.get("inbox") or []
